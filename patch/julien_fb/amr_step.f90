recursive subroutine amr_step(ilevel,icount)
  ...
  if(ilevel==levelmin)then
     !----------------------------------------------------
     ! Kinetic feedback from giant molecular clouds !MT!
     !----------------------------------------------------
     if(hydro.and.star.and.eta_sn>0.and.f_w>0) then
        ! Yohan Dubois 2008 kinetic feedback routine
        ! call kinetic_feedback
        ! Taysun Kimm 2014 momentum feedback routine
        call kinetic_feedback_cell
     endif
