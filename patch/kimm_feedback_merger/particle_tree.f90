...
subroutine fill_comm(ind_part,ind_com,ind_list,np,ilevel,icpu)
  ...
  if(star.or.sink)then
     ...
     do i=1,np
        reception(icpu,ilevel)%up(ind_com(i),current_property)=st_n_tp(ind_part(i)) !SD
        reception(icpu,ilevel)%up(ind_com(i),current_property+1)=st_n_SN(ind_part(i)) !SD
        reception(icpu,ilevel)%up(ind_com(i),current_property+2)=st_e_SN(ind_part(i)) !SD
     end do
...
subroutine empty_comm(ind_com,np,ilevel,icpu)
  ...
  if(star.or.sink)then
     ...
     do i=1,np
        ...
        st_n_SN(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property) !SD
        st_e_SN(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property) !SD
     end do
     current_property=current_property+3
     if(use_initial_mass)then
        do i=1,np
