...
subroutine output_frame()
  ...
  real(kind=8),dimension(:,:,:),allocatable::rt_data_frame,rt_data_frame_all !RTMOVIE
 ...
 do proj_ind=1,LEN(trim(proj_axis)) 
  ...
  if(rt) then
     do ll=1,NGROUPS
        ...
        !if(myid==1) print*,'moviefiles ',ll,rt_moviefiles(ll)
     end do
  endif
  ! END RTMOVIES
#endif
  ...
  if(proj_axis(proj_ind:proj_ind).eq.'x')then
    ...
    delx=min(2*min(xcen,ycen,zcen,boxlen-xcen,boxlen-ycen,boxlen-zcen),deltay_frame(proj_ind*2-1)+deltay_frame(proj_ind*2)/aexp)
    dely=min(2*min(xcen,ycen,zcen,boxlen-xcen,boxlen-ycen,boxlen-zcen),deltaz_frame(proj_ind*2-1)+deltaz_frame(proj_ind*2)/aexp)
    delz=min(2*min(xcen,ycen,zcen,boxlen-xcen,boxlen-ycen,boxlen-zcen),deltax_frame(proj_ind*2-1)+deltax_frame(proj_ind*2)/aexp)
  elseif(proj_axis(proj_ind:proj_ind).eq.'y')then
    ...
    delx=min(2*min(xcen,ycen,zcen,boxlen-xcen,boxlen-ycen,boxlen-zcen),deltax_frame(proj_ind*2-1)+deltax_frame(proj_ind*2)/aexp)
    dely=min(2*min(xcen,ycen,zcen,boxlen-xcen,boxlen-ycen,boxlen-zcen),deltaz_frame(proj_ind*2-1)+deltaz_frame(proj_ind*2)/aexp)
    delz=min(2*min(xcen,ycen,zcen,boxlen-xcen,boxlen-ycen,boxlen-zcen),deltay_frame(proj_ind*2-1)+deltay_frame(proj_ind*2)/aexp)
  else
    xcen=xcentre_frame(proj_ind*4-3)+xcentre_frame(proj_ind*4-2)*aexp+xcentre_frame(proj_ind*4-1)*aexp**2+xcentre_frame(proj_ind*4)*aexp**3
    ...
    delx=min(2*min(xcen,ycen,zcen,boxlen-xcen,boxlen-ycen,boxlen-zcen),deltax_frame(proj_ind*2-1)+deltax_frame(proj_ind*2)/aexp)
    dely=min(2*min(xcen,ycen,zcen,boxlen-xcen,boxlen-ycen,boxlen-zcen),deltay_frame(proj_ind*2-1)+deltay_frame(proj_ind*2)/aexp)
    delz=min(2*min(xcen,ycen,zcen,boxlen-xcen,boxlen-ycen,boxlen-zcen),deltaz_frame(proj_ind*2-1)+deltaz_frame(proj_ind*2)/aexp)
  endif
              ...
              if(ok(i))then
                 ...
                 do ii=imin,imax
                    ...
                    do jj=jmin,jmax
                       ...
                       ! Density weighted average density (joki added the scale_nH)
                       !data_frame(ii,jj,1)=data_frame(ii,jj,1)+dvol*max(uold(ind_cell(i),1),smallr)**2
                       data_frame(ii,jj,1)=data_frame(ii,jj,1)+dvol*max(uold(ind_cell(i),1),smallr)**2 * scale_nH
#ifdef SOLVERmhd
                       do kk=2,NVAR+3
#else                       
                       do kk=2,NVAR
...
!  end do
  ...
  call MPI_ALLREDUCE(data_frame,data_frame_all,nw_frame*nh_frame*(NVAR+6+1) &
       ,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
#else
  allocate(data_frame_all(1:nw_frame,1:nh_frame,0:NVAR+2))
  call MPI_ALLREDUCE(data_frame,data_frame_all,nw_frame*nh_frame*(NVAR+2+1) &
       ,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
#endif
  allocate(dens_all(1:nw_frame,1:nh_frame))
  call MPI_ALLREDUCE(dens,dens_all,nw_frame*nh_frame,MPI_DOUBLE_PRECISION &
       ,MPI_SUM,MPI_COMM_WORLD,info)
  allocate(vol_all(1:nw_frame,1:nh_frame))
  call MPI_ALLREDUCE(vol,vol_all,nw_frame*nh_frame,MPI_DOUBLE_PRECISION   &
       ,MPI_SUM,MPI_COMM_WORLD,info)
  ...
  do ii=1,nw_frame
    do jj=1,nh_frame
      ...
      if(rt) then
         do kk=1,NGROUPS
            if(rt_movie_vars(kk).eq.1) rt_data_frame(ii,jj,kk) = rt_data_frame(ii,jj,kk) / dens(ii,jj)
         end do
      endif
      ! END RTMOVIES
#endif
      ...
      if(rt) then
         do kk=1, NGROUPS
            if (rt_movie_vars(kk).eq.1) then
               ...
               write(ilun)data_single
               close(ilun)
               !print*,'outputting rt frame ', kk , rt_moviefiles(kk),' vars= ',rt_movie_vars
            end if
         end do
      endif
      ! END RTMOVIES
