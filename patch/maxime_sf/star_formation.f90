...
subroutine star_formation(ilevel)
  ...
  use pm_commons
  use hydro_commons
  use cooling_module, ONLY: XH=>X, rhoc, mH, twopi !MT!
  use random
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
  ...
  integer ::ntot,ntot_all,info,nstar_corrected,ncell!ideb,ndeb
  logical ::ok_free,ok_all
  real(dp)::d,x,y,z,u,v,w,e,zg,vdisp,dgas,theta!MT!
  real(dp)::mstar,dstar,tstar,nISM,nCOM,temp,c_s2,phi_t,e_cts,sigs,scrit!MT!
  real(dp)::T2,nH,T_poly
  real(dp)::velc,uc,vc,wc,mass_load,ul,ur,trgv,alpha0,lamjt!MT!
  real(dp)::vxgauss,vygauss,vzgauss,birth_epoch,factG!MT!
  real(dp),parameter :: pi=0.5*twopi!MT!
  ...
  real(dp)::dx,dx_loc,scale,vol_loc,dx_min,vol_min,d1,d2,d3,d4,d5,d6!MT!
  !real(dp)::mdebris,vdebris,zdebris,rdebris !MT!
  ...
  !integer ,dimension(1:nvector),save::list_debris,ind_debris !MT!
           ...
           do i=1,ngrid
              ...
              if (ok(i)) then 
                 ...
                 if (d <= d0) then
                    ...
                    lamjt = (pi*trgv + sqrt(pi*pi*trgv*trgv + 36.0*pi*c_s2*factG*d*dx_loc**2))/(6.0*factG*d*dx_loc**2)
                    if (lamjt > 1d0) then ! Jeans length resolved: gas is stable 
                       ...
                       alpha0 = 5d0/(pi*factG*d)*(trgv + c_s2)/dx_loc**2
     ...
     do ind=1,twotondim
        ...
        do i=1,ngrid
           if(flag2(ind_cell(i))>0)then
              ...
              uold(ind_cell(i),1) = d-n*dstar !MT!
           endif
        end do
