subroutine read_params
  ...
  namelist/output_params/noutput,foutput,fbackup,aout,tout,output_mode &
       & ,tend,delta_tout,aend,delta_aout,gadget_output,write_onefile_time
...
81 continue
  ...
  if (myid==1 .and. nrestart .gt. 0) then
     ...
     inquire(file=info_file, exist=ok) 
     do while(.not. ok .and. nrestart .gt. 1)
        nrestart = nrestart - 1
        call title(nrestart,nchar)
        info_file='output_'//TRIM(nchar)//'/info_'//TRIM(nchar)//'.txt'
        inquire(file=info_file, exist=ok) 
  ...
  if(imovout>0) then
     ...
     if (myid == 1) then
       write(6,*) " Movie on: movie_variables are ", movie_vars
       write(6,*) " Movie on: proj_axis are ", proj_axis
     endif
  endif
  !--------------------------------------------------
