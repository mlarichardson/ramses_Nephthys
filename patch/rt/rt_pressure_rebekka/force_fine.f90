...
subroutine force_fine(ilevel,icount)
  ...
  use pm_commons
  use poisson_commons
  use rt_cooling_module !rtPress_force
  use rt_parameters !rtPress_force
  use hydro_commons !rtPress_force
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
#endif
        ...
        do ind=1,twotondim
           ...
           do idim=1,ndim
              do i=1,ngrid
                 ...
                 if(rtPress_force) then  ! Add radiation pressure force:   !RTpress
                    f(ind_cell(i),idim) = f(ind_cell(i),idim) &            !RTpress
                                          + uold(ind_cell(i),iIons+idim+2) !RTpress
                 endif                                                     !RTpress
              end do
           end do
