...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  use rt_cooling_module, only: rt_solve_cooling,iIR,rt_isIRtrap &
       ,rt_pressBoost,iIRtrapVar,kappaSc,a_r,is_kIR_T,rt_vc,rtPress_force
#endif
  implicit none
  ...
  do ind=1,twotondim
...
#ifdef RT
     if(rtPress_force) p_gas(:,:)=0d0 ! Use gravity force for radiation pressure
     if(neq_chem) then
...
#ifdef RT
     if(.not. static .and. .not. rtPress_force) then
     ...
     if(.not. static .and. rtPress_force) then
        ! p_gas contains the momentum CHANGE.
        ! Convert this into acceleration and store for using in force_fine:
        do i=1,nleaf
           uold(ind_leaf(i),iIons+3:iIons+3+ndim-1) = &
                p_gas(i,1:ndim)/MAX(uold(ind_leaf(i),1),smallr) &
                /scale_d/scale_v/dtnew(ilevel)
        end do
     endif
#endif
