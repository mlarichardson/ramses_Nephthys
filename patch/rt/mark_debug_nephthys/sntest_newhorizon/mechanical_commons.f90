...
module mechanical_commons
  ...
  ! ex) Geen et al (2015) (w/ photoionisation) 
  !     A_SN = 5.18e5, alpha ~ 0, gamma ~ 0
  ! TK
  !     A_SN = 5.18e5, alpha = 0, beta = 16/17, gamma = -0.14
!   real(dp) :: A_pop3 = 3d5
...
!   real(dp) :: expZ_pop3 = -0.14
   ...
   real(dp),parameter:: expN_SN_boost=-0.15   ! empirical estimate
   real(dp),parameter:: expE_SN_boost=16d0/17d0
   real(dp),parameter:: expZ_SN=-0.14
   real(dp)::porosity = 1.0
   ...
   real(dp),dimension(1:ncomm_max)::eSN_comm          ! energy of SNe normalised by 10^51 erg (for PopIII) 
