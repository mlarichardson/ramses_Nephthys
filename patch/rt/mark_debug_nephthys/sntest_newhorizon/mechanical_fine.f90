        ...
        if(npart1>0)then
           ...
           do jpart=1,npart1
              ...
              if(sn2_real_delay)then
                 ...
                 if (idp(ipart).le.0.and.tp(ipart).ne.0.0d0.and.tp(ipart).ge.tyoung) then
                 ...
                 if (idp(ipart).le.0.and.tp(ipart).ne.0.0d0.and.tp(ipart).le.tyoung)then
                    ok=.true.
                 endif
              endif
...
398     format('MFB = ',f7.3,1x,f7.3,1x,f5.1,1x,f5.3,1x,f9.5,1x,f7.3)
     ...
     do j=1,nSNnei
        ...
        if(cpu_map(father(igrid)).eq.myid) then ! if belong to myid
           ...
           !print *, 'fw,fcrit =', sngl(f_w_cell/f_w_crit), sngl(p_solid(i,j)*scale_msun*vol_loc*scale_l/scale_t/1d5),sngl(boost_geen_ad),sngl(f_esn2)
