...
subroutine make_tree_fine(ilevel)
  ...
  logical::debug_now=.true.
!  integer::ilevel_debug=18
!  integer::myid_debug=242
!  integer::step_debug=46262
  integer::ilevel_debug=18
  integer::myid_debug=244
  integer::step_debug=47303
  ...
  do icpu=1,ncpu
     ...
     if (debug_now .and. (nstep .ge. step_debug) .and. (ilevel.ge.ilevel_debug-1) ) then
       if (icpu .eq. myid .and. icpu .eq. myid_debug) then
         write(6,'("Debug Particle Tree A: ",5i)') myid, icpu, igrid,numbl(icpu,ilevel),numbp(max(igrid,1))
       endif
     endif 
     ig=0
     ip=0
...
subroutine check_tree(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  ...
  logical::debug_now=.true.
!  integer::ipart_debug=3116
!  integer::ilevel_debug=18
!  integer::step_debug=46262
!  integer::igrid1_debug=4985,igrid2_debug=4679
  integer::ipart_debug=7124
  integer::ilevel_debug=18
  integer::step_debug=47303
  integer::igrid1_debug=7461,igrid2_debug=7067
  integer::ig1_debug=-1,ig2_debug=-1,ip_debug=-1,ipg_debug=-1
  ...
  if (debug_now .and. (nstep .ge. step_debug) .and. (ilevel.ge.ilevel_debug-1) ) then
    do i=1,ng
      if (ind_grid(i) .eq. igrid1_debug) ig1_debug = i
      if (ind_grid(i) .eq. igrid2_debug) ig2_debug = i
    enddo
    do i=1,np
      if (ind_part(i) .eq. ipart_debug) then
        ip_debug = i
        ipg_debug = ind_grid_part(i)
      endif
    enddo
    if (ip_debug .ne. -1) then
      write(6,'("Check Tree Debug A: ",10i,es16.8)') myid,nstep,ilevel,ng,np,ip_debug,ipg_debug,ind_grid(ipg_debug),ig1_debug,ig2_debug,dx
      write(6,'("Check Tree Debug B1: ",2i,3es16.8)') myid, ipart_debug, xp(ipart_debug,:) 
      write(6,'("Check Tree Debug B2: ",2i,3es16.8)') myid, ind_grid(ipg_debug), xg(ind_grid(ipg_debug),:)
      write(6,'("Check Tree Debug C1: ",2i,3es16.8)') myid, igrid1_debug, xg(igrid1_debug,:)
      write(6,'("Check Tree Debug C2: ",2i,3es16.8)') myid, igrid2_debug, xg(igrid2_debug,:)
      write(6,'("Check Tree Debug D: ",4i)') myid,father(ind_grid(ipg_debug)),father(igrid1_debug),father(igrid2_debug)
    endif
    if (ig2_debug .ne. -1) then
      write(6,'("Check Tree Debug AA: ",9i,es16.8)') myid,nstep,ilevel,ng,np,ip_debug,ipg_debug,ig1_debug,ig2_debug,dx
      write(6,'("Check Tree Debug CC1: ",2i,3es16.8)') myid, igrid1_debug, xg(igrid1_debug,:)
      write(6,'("Check Tree Debug CC2: ",2i,3es16.8)') myid, igrid2_debug, xg(igrid2_debug,:)
    endif
  endif
  ...
  if (debug_now .and. (nstep .ge. step_debug) .and. (ilevel.ge.ilevel_debug-1) ) then
    if (ip_debug .ne. -1) then
      write(6,'("Check Tree Debug E: ",i,2L2,i)') myid,error,ok(ip_debug),ind_son(ip_debug) 
    endif
  endif
  if(error)then
  ...
  if (debug_now .and. (nstep .ge. step_debug) .and. (ilevel.ge.ilevel_debug-1) ) then
    if (ip_debug .ne. -1) then
      write(6,'("Check Tree Debug F: ",i,27(i,i,i))') myid,(j,nbors_father_cells(ind_grid_part(ip_debug),j), &
                          & son(nbors_father_cells(ind_grid_part(ip_debug),j)),j=1,27)
      write(6,'("Check Tree Debug G: ",i,8(i,i,i))') myid,(j,nbors_father_grids(ind_grid_part(ip_debug),j), &
                          & cpu_map(nbors_father_grids(ind_grid_part(ip_debug),j)),j=1,8)
    endif
  endif
...
subroutine virtual_tree_fine(ilevel)
  ...
  if(star.or.sink) then
     ...
     if (nchem .gt. 0) particle_data_width = particle_data_width + nchem
  endif
