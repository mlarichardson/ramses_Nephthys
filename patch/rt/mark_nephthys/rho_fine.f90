...
subroutine rho_fine(ilevel,icount)
  ...
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v,ek,val
  ...
  ! Extra refine variables:
  integer::delta_lvl,lmax2,i_refinevar,ivar_refine2
  real(kind=8)::var_cut_refine2
  logical::ok_cell2
  ...
  ! Set minimum level for extra refine region. This is assuming there are stars or
  ! sinks where the maximum permitted level is enforced to maintain a fixed physical
  ! resolution
  
  if (refine_extra > 0) then
    delta_lvl = 0
    ! 0.7937005259840998**3 = 0.5
    do while ( 0.7937005259840998/2.d0**delta_lvl .gt. aexp)
      delta_lvl = delta_lvl + 1
    enddo
    lmax2 = nlevelmax-delta_lvl - delta_lvl_extra_refine
  endif
           ...
           if(ivar_refine>0)then
              do i=1,active(ilevel)%ngrid
                 ...
                 if(scalar>var_cut_refine)then
                    ...
                    if (refine_extra > 0 .and. ilevel .lt. lmax2) then
                      ! Here I should cycle through the extra number of refine variables
                      ! and fill in a second work array. This will also need to be shared 
                      ! the same way phi is. 
                      ok_cell2 = .false.
                      do i_refinevar=1,refine_extra
                        ivar_refine2 = ivar_refine_extra(i_refinevar)
                        var_cut_refine2 = var_cut_refine_extra(i_refinevar)
                        ! by default if ivar_refine2 = 1 I'm assuming it's an overdensity
                        ! by default if ivar_refine2 = 5 I'm assuming temperature
                        ! by default if ivar_refine2 = 2-4 I'm assuming greater than abs value
                        if (ivar_refine2 .eq. 1) then
                          if (cosmo) then
                            val = sign(scale_d,var_cut_refine2)*uold(active(ilevel)%igrid(i)+iskip,1) / &
                                       (rhoc*(h0/100.d0)**2*(omega_m/aexp**3 + omega_l)*omega_b/omega_m) !looking at gas density
                          else
                            val = sign(scale_d,var_cut_refine2)*uold(active(ilevel)%igrid(i)+iskip,1)
                          endif 
                        else if (ivar_refine2 .eq. ndim+2) then
                          ek = 0.d0
                          do idim=1,ndim
                            ek = ek + uold(active(ilevel)%igrid(i)+iskip,1+idim)**2
                          enddo
                          ek = 0.5d0*ek/uold(active(ilevel)%igrid(i)+iskip,1)
                          val = sign(scale_T2,var_cut_refine2)*(uold(active(ilevel)%igrid(i)+iskip,5)-ek)/ &
                                         uold(active(ilevel)%igrid(i)+iskip,1)*(gamma-1.0)
                        else if (ivar_refine2 .eq. ndim+3) then
                          val = sign(1.d0,var_cut_refine2)*uold(active(ilevel)%igrid(i)+iskip,3+ndim) / &
                                         uold(active(ilevel)%igrid(i)+iskip,1)
                        else if (ivar_refine2 .gt. 1 .and. ivar_refine2 .lt. ndim+2) then
                          val = sign(1.d0,var_cut_refine2)*abs(scale_l/scale_t*uold(active(ilevel)%igrid(i)+ &
                                       iskip,ivar_refine2)/uold(active(ilevel)%igrid(i)+iskip,1))
                        else
                          stop "Error. var_refine_extra unrecognized."
                        endif  
                        if ( val > var_cut_refine2) then
                          if (i_refinevar == 1) then
                             ok_cell2 = .true.
                          else
                             if (refine_extra_or) then
                               ok_cell2 = .true.
                             else
                               ok_cell2 = ok_cell2 .and. .true. 
                               ! equivalent to ok_cell2=ok_cell2
                             endif
                          endif
                        endif
                      enddo  
                      ...
                      if (ok_cell2) then
                         ! cpu_map2 may very well be set to 1 due to lagrangian mass
                         ! constraint. But if it's set to 1 here it also needs to be
                         ! refined.
     ...
     do ind=1,twotondim
        ...
        do i=1,active(ilevel)%ngrid
           if(phi(active(ilevel)%igrid(i)+iskip)>=m_refine(ilevel))then
              cpu_map2(active(ilevel)%igrid(i)+iskip)=1
           else if (refine_extra == 0) then

!    NOTE!! I initialize cpu_map2 to zero over the same region, to allow for two separate
!            conditions that can allow cpu_map2 to be set to 1.
              cpu_map2(active(ilevel)%igrid(i)+iskip)=0
           end if
        end do
     end do
