...
subroutine condinit(x,u,dx,nn)
  ...
  a2=1d4 / scale_T2 ! sound speed squared !-joki 
  aa=sqrt(a2)
  ...
  do i=1,nn
    ...
    ! Projected cell position over galactic centers axis
     ...
     if(((r-dx/2.0D0).lt.rdisk) .and. ((abs_z-dx/2.0D0) .lt. HH_max)) then
        ...
        weight = (min(r+dx/2.0D0,rdisk)-(r-dx/2.0D0))/dx ! Fraction of the cell inside the cut
        ...
        q(i,1)= dmin !1d-5/scale_nH                                   !joki
        q(i,ndim+2)= 1d6/scale_T2*q(i,1)                              !joki
        ! V = Vgal                                                    !joki
        q(i,ndim-1:ndim+1)= vgal                                      !joki
        if(metal)q(i,iMetal)=1d-4 * 0.02                              !joki
