subroutine init_part
  ...
  if(star.or.sink)then
     ...
     tp=0.0
     allocate(st_n_tp(npartmax))                                             !SD
     st_n_tp=0.0                                                             !SD
     allocate(st_n_SN(npartmax))                                             !SD
     st_n_SN=0.0                                                             !SD
     allocate(st_e_SN(npartmax))                                             !SD
     st_e_SN=0.0                                                             !SD
     if(metal)then
  ...
  if(nrestart>0)then
...
#endif
        read(ilun)xdp                                                     !SD
        st_n_tp(1:npart2)=xdp                                             !SD
        read(ilun)xdp                                                     !SD
        st_n_SN(1:npart2)=xdp                                             !SD
        read(ilun)xdp                                                     !SD
        st_e_SN(1:npart2)=xdp                                             !SD
        if(metal)then
           ! Read metallicity
           read(ilun)xdp
           zp(1:npart2)=xdp
     ...
     case ('grafic')
        ...
        if(star.or.sink)then
           do ipart=1,npart
              tp(ipart)=0d0
              st_n_tp(ipart)=0d0 !----------------------------------------!SD
              st_n_SN(ipart)=0d0 !----------------------------------------!SD
              st_e_SN(ipart)=0d0 !----------------------------------------!SD
              if(metal)then
                 zp(ipart)=0d0
              end if
           end do
